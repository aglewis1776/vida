...
<div class="overflow-hidden rounded-lg bg-white shadow-sm border border-gray-200 mb-2">
	...
	<div class="flex items-center p-4">
		<div class="flex h-10 w-10 flex-col items-center justify-center rounded-md bg-gray-200 text-gray-700 mr-3">
			<span class="text-base font-bold">{new Date(bill.dueDate + 'T00:00:00').getDate()}</span>
			<span class="text-[10px] font-semibold">{new Date(bill.dueDate + 'T00:00:00').toLocaleDateString('pt-BR', { month: 'short' }).toUpperCase().replace('.', '')}</span>
		</div>
		...
		<div class="text-right ml-2">
			<p class="font-bold text-lg text-gray-800">R$ {bill.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
			<p class="text-xs {dueDateStatus.color}">{dueDateStatus.text}</p>
		</div>
		<div class="flex flex-col items-end ml-4 gap-1">
			...
			<button class="btn btn-xs btn-ghost btn-circle text-error" aria-label="Excluir" title="Excluir" on:click={deleteBill}>
			...
			<button class="btn btn-xs btn-primary btn-circle" aria-label="Pagar com Pix" title="Pagar com Pix" on:click={payBill}>
