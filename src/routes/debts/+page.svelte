...
<script lang="ts">
	...
	let debts = liveQuery<Debt[]>(async () => {
		...
		return await db.debts.where('profileId').equals(profile.id).sortBy('priority');
	});
	...
	function handleDebtSaved() {
		// The liveQuery automatically updates the list, so we just need to log it.
	...
	function getCategoryColor(category: Debt['category']) {
		switch (category) {
			...
			case 'Financiamento de Veículo':
				return 'bg-blue-500';
			case 'Financiamento Imobiliário':
				return 'bg-green-600';
			case 'Contas Atrasadas':
				return 'bg-amber-600';
			case 'Crediário / Carnê':
				return 'bg-pink-500';
			case 'Agiota / Empréstimo Informal':
				return 'bg-black';
			case 'Família / Amigos':
				return 'bg-teal-500';
			default:
		...
		{#if $debts && $debts.length > 0}
			{#each $debts as debt (debt.id)}
				<div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
					{#if debt.category}
						<div class="px-4 py-1 text-left font-semibold text-white {getCategoryColor(debt.category)}">
							{debt.category}
						</div>
					{/if}
...
<!-- This component now receives functions as props -->
<AddDebtModal 
	bind:showModal={showAddEditModal} 
	{debtToEdit} 
